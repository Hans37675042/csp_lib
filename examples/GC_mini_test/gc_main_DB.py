...
async def main():
    ...
    influx_db = InfluxDBClient3(host="http://localhost:8181", token=os.environ.get("INFLUXDB_TOKEN", ""), org="my-org", database="demo")
    ...
    cancel_rc    = device.on("read_complete",    on_read_complete)
 
    # ----------------------------------------------------------
    # . 設定 UnifiedDeviceManager（帶 MongoDB 上傳）
    # ----------------------------------------------------------
    ...
    controller = SystemController(
        ...
        config=SystemControllerConfig(
            context_mappings=[
                ...
                ContextMapping(point_name="active_power", context_field="extra.meter_power", trait="pcs"),
            ],
            command_mappings=[
                CommandMapping(point_name="p_set", command_field="p_target", trait="pcs"),
                CommandMapping(point_name="q_set", command_field="q_target", trait="pcs"),
            ],
            system_base=SystemBase(p_base=10, q_base=10),
            protection_rules=[
                SOCProtection(SOCProtectionConfig(soc_high=90.0, soc_low=70.0, warning_band=5.0)),
            ...
            heartbeat_mappings=[
                HeartbeatMapping(point_name="heartbeat", trait="pcs", mode=HeartbeatMode.INCREMENT, increment_max=255),],
            capacity_kva=10
        )
    )
    controller.orchestrator.register(startup_cmd)

    # ----------------------------------------------------------
    # . 加入策略
    # ----------------------------------------------------------
    pq_strategy = PQModeStrategy(PQModeConfig(p=0.0, q=0.0))
    controller.register_mode(
        "pq", pq_strategy, ModePriority.MANUAL,
    )
    ...
    # ----------------------------------------------------------
    try:
        async with manager, controller:
            logger.info(f"UnifiedDeviceManager狀態: {manager.is_running}, SystemController狀態: {controller.is_running}")
            logger.info(f"開始讀取設備並上傳至 DB {MONGO_DATABASE}.{MONGO_PCS_COLLECTION}")
            await controller.orchestrator.execute("startup_sequence")  # 執行啟動序列指令
            await asyncio.Event().wait()  # 策略更新由 read_complete 事件驅動

    finally:
        # ----------------------------------------------------------
        # . 清理：flush 剩餘資料、關閉連線、取消事件監聽
        # ----------------------------------------------------------
